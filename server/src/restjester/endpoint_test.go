...
func getSampleEndpoint(index int) endpoint {
	...
	ep = createEndpoint("/a/b/c", query, "POST", 200, "data a")
	samples = append(samples, ep)

	query = map[string][]string{"b": []string{"2"}}
	ep = createEndpoint("/d/e/f", query, "GET", 200, "data b")
	samples = append(samples, ep)

	query = map[string][]string{"c": []string{"3"}}
	ep = createEndpoint("/g/h/i", query, "POST", 200, "data c")
	samples = append(samples, ep)

	query = map[string][]string{"d": []string{"4"}}
	ep = createEndpoint("/j/k/l", query, "POST", 200, "data d")
	samples = append(samples, ep)
...
func TestCreateEndpoint(t *testing.T) {
	...
	query := map[string][]string{"a": []string{"1"}}
	endpoint := createEndpoint("/a/b/c", query, "POST", 200, "data a")
	...
	dataCheck := endpoint.Data == "data a"
